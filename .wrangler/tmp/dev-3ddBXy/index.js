...
// .wrangler/tmp/bundle-DzN4xa/checked-fetch.js
...
async function discoverCandidates(db, agentId) {
  try {
    ...
    for (const id of topIds) {
      ...
      const existing = await db.prepare(
        "SELECT id FROM sources WHERE agent_id = ? AND url = ?"
      ).bind(agentId, item.url).first();
      if (existing) continue;
      const sourceId = crypto.randomUUID();
      const now = (/* @__PURE__ */ new Date()).toISOString();
      await db.prepare(
        "INSERT INTO sources (id, agent_id, url, source_type, title, status, discovered_at) VALUES (?, ?, ?, 'HACKER_NEWS', ?, 'DISCOVERED', ?)"
      ).bind(sourceId, agentId, item.url, item.title, now).run();
      const candidateId = crypto.randomUUID();
      await db.prepare(
        "INSERT INTO candidates (id, agent_id, title, status, discovered_at) VALUES (?, ?, ?, 'PENDING', ?)"
      ).bind(candidateId, agentId, item.title, now).run();
      await db.prepare(
        "INSERT INTO candidate_sources (candidate_id, source_id, relationship) VALUES (?, ?, 'PRIMARY')"
      ).bind(candidateId, sourceId).run();
    }
  } catch (error) {
...
// src/editorial/index.ts
var import_checked_fetch29 = __toESM(require_checked_fetch());
async function evaluateCandidates(db, agentId, llm, cycleId) {
  const { results: candidates } = await db.prepare(
    "SELECT id, title FROM candidates WHERE agent_id = ? AND status = 'PENDING'"
  ).bind(agentId).all();
  ...
  for (const candidate of candidates) {
    try {
      const decision = await llm.generateStructured(
        `EDITORIAL_JUDGMENT: Evaluate if this technology topic is worth publishing based on second-order effects. Topic: ${candidate.title}`,
        { decision: "PUBLISH|REJECT", rationale: "string", score: "number" }
      );
      const decisionId = crypto.randomUUID();
      await db.prepare(
        "INSERT INTO editorial_decisions (id, agent_id, candidate_id, decision, rationale, score, decided_at) VALUES (?, ?, ?, ?, ?, ?, ?)"
      ).bind(decisionId, agentId, candidate.id, decision.decision, decision.rationale, decision.score, now).run();
      if (decision.decision === "PUBLISH" && decision.score > 0.7) {
        await db.prepare("UPDATE candidates SET status = 'APPROVED' WHERE id = ?").bind(candidate.id).run();
...
__name(evaluateCandidates, "evaluateCandidates");

// src/content/index.ts
var import_checked_fetch30 = __toESM(require_checked_fetch());
async function generateContent(db, agentId, candidate, llm, cycleId) {
  try {
    const { results: sources } = await db.prepare(
      "SELECT s.id, s.url FROM candidate_sources cs JOIN sources s ON cs.source_id = s.id WHERE cs.candidate_id = ?"
    ).bind(candidate.id).all();
    if (sources.length === 0) return false;
    const postData = await llm.generateStructured(
      `CONTENT_GENERATION: Write an insightful, thesis-driven post about the following topic. Do not just summarize. Focus on second-order consequences. Topic: ${candidate.title}`,
    ...
    ).bind(postId, agentId, candidate.id, postData.text, postData.rationale, now, now).run();
    ...
    ).bind(eventId, agentId, postId, now).run();
    return true;
  } catch (error) {
...
// src/runtime/scheduler.ts
async function runAutonomousCycle(db, agentId, llm) {
  ...
  const lockedUntil = new Date(now.getTime() + 5 * 60 * 1e3).toISOString();
  await db.prepare("UPDATE runtime_state SET state = 'RUNNING', locked_until = ?, updated_at = ? WHERE agent_id = ?").bind(lockedUntil, now.toISOString(), agentId).run();
  ...
  try {
    ...
    await discoverCandidates(db, agentId);
    console.log(`Evaluating candidates for agent ${agentId}`);
    const approvedCandidates = await evaluateCandidates(db, agentId, llm, cycleId);
    console.log(`Generating content for ${approvedCandidates.length} approved candidates`);
    let publishedCount = 0;
    for (const candidate of approvedCandidates) {
      const published = await generateContent(db, agentId, candidate, llm, cycleId);
      if (published) publishedCount++;
    }
    const finishedAt = (/* @__PURE__ */ new Date()).toISOString();
    await db.prepare("UPDATE runtime_cycles SET status = 'COMPLETED', completed_at = ?, posts_published = ? WHERE id = ?").bind(finishedAt, publishedCount, cycleId).run();
    await db.prepare("UPDATE runtime_state SET state = 'IDLE', locked_until = NULL, last_successful_cycle_at = ?, updated_at = ? WHERE agent_id = ?").bind(finishedAt, finishedAt, agentId).run();
  } catch (error) {
    console.error("Cycle Error:", error);
    const failedAt = (/* @__PURE__ */ new Date()).toISOString();
    await db.prepare("UPDATE runtime_cycles SET status = 'FAILED', error_message = ?, completed_at = ? WHERE id = ?").bind(String(error), failedAt, cycleId).run();
    await db.prepare("UPDATE runtime_state SET state = 'ERROR', locked_until = NULL, last_failed_cycle_at = ?, updated_at = ? WHERE agent_id = ?").bind(failedAt, failedAt, agentId).run();
  }
}
...
var MockLLMProvider = class {
  static {
    __name(this, "MockLLMProvider");
  }
  async generate(prompt, model) {
    return "Mock generated text";
  }
  async generateStructured(prompt, schema, model) {
...
var app = new Hono2();
app.get("/", (c) => {
  return c.text("Second Order Agent API is running.");
});
app.post("/internal/runtime/wake", async (c) => {
  try {
    const { results: agents } = await c.env.DB.prepare("SELECT id FROM agents WHERE status = 'ACTIVE'").all();
    const llm = new MockLLMProvider();
    for (const agent of agents) {
      await runAutonomousCycle(c.env.DB, agent.id, llm);
    }
...
var src_default = {
  ...
  async scheduled(event, env, ctx) {
    ...
    const { results: agents } = await env.DB.prepare("SELECT id FROM agents WHERE status = 'ACTIVE'").all();
    const llm = new MockLLMProvider();
    for (const agent of agents) {
      ctx.waitUntil(runAutonomousCycle(env.DB, agent.id, llm));
    }
  }
};
