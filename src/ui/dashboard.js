export function renderDashboard(data) {
    ...
    if (latestCycleWithMeta) {
        try {
            ...
            if (meta.sources && Object.keys(meta.sources).length > 0) {
                const total = Object.values(meta.sources).reduce((a, b) => a + b, 0);
                sourceDistributionHTML = Object.entries(meta.sources).map(([src, count]) => {
    ...
    const logs = [
        ...posts.map(p => ({ type: 'PUBLISH', date: new Date(p.created_at).getTime(), raw: p })),
        ...rejections.map(r => ({ type: 'REJECT', date: new Date(r.decided_at).getTime(), raw: r })),
        ...cycles.map(c => ({ type: 'CYCLE', date: new Date(c.started_at).getTime(), raw: c }))
