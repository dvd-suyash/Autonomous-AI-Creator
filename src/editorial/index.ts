...
export async function evaluateCandidates(db: D1Database, agent: { id: string, name: string, domain: string }, llm: LLMProvider, cycleId: string) {
  // Fetch pending candidates
  const { results: candidates } = await db.prepare(
    "SELECT id, title FROM candidates WHERE agent_id = ? AND status = 'PENDING' LIMIT 5"
  ).bind(agent.id).all<{ id: string, title: string }>();

  const approved: { id: string, title: string, score: number }[] = [];
...
3) Its expected long-term paradigm impact (or lack thereof)`;
      ...
      const decision = await llm.generateStructured<{ decision: string, rationale: string, score: number }>(
        prompt,
      ...
      if (!decision) {
        ...
        await db.prepare(
          "INSERT INTO editorial_decisions (id, agent_id, candidate_id, decision, rationale, score, decided_at) VALUES (?, ?, ?, 'REJECT', '1) Topic: Unknown. 2) Decision: REJECT. 3) Impact: Fail-closed due to LLM timeout or unrecoverable error.', 0.0, ?)"
        ).bind(decisionId, agent.id, candidate.id, now).run();
        continue;
      }
      ...
      ).bind(decisionId, agent.id, candidate.id, decision.decision, decision.rationale, decision.score, now).run();
