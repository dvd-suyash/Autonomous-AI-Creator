...
export async function discoverCandidates(db: D1Database, agentId: string): Promise<DiscoveryStats> {
  const stats: DiscoveryStats = { totalScanned: 0, newInserted: 0, sourceBreakdown: {} };

  try {
    const candidatesToInsert: { url: string, title: string, source_type: string }[] = [];

    // Helper to fetch a reddit feed
    async function fetchReddit(subreddit: string, sort: string, limit: number, minScore: number, sourceType: string) {
      try {
        const url = sort === 'new'
      ...
      } catch (e) { console.error(`Reddit ${subreddit}/${sort} Error`, e); }
    }
    ...
    try {
      ...
      if (hnRes.ok) {
        const ids = (await hnRes.json() as number[]).slice(0, 3);
        for (const id of ids) {
    ...
    } catch (e) { console.error('HN Top Error', e); }
    ...
    try {
      ...
      if (hnNewRes.ok) {
        const ids = (await hnNewRes.json() as number[]).slice(0, 3);
        for (const id of ids) {
          const itemRes = await fetch(`https://hacker-news.firebaseio.com/v0/item/${id}.json`);
          if (itemRes.ok) {
            const item = await itemRes.json() as any;
    ...
    } catch (e) { console.error('HN Best Error', e); }
    ...
    try {
      ...
      if (devRes.ok) {
        const articles = await devRes.json() as any[];
    ...
    for (const item of candidatesToInsert) {
      ...
      const existing = await db.prepare(
        "SELECT id FROM sources WHERE agent_id = ? AND url = ? AND discovered_at > datetime('now', '-2 hours')"
      ...
      await db.prepare(
        "INSERT INTO sources (id, agent_id, url, source_type, title, status, discovered_at) VALUES (?, ?, ?, ?, ?, 'DISCOVERED', ?)"
      ).bind(sourceId, agentId, item.url, item.source_type, cleanTitle, now).run();

      const candidateId = crypto.randomUUID();
      await db.prepare(
        "INSERT INTO candidates (id, agent_id, title, status, discovered_at) VALUES (?, ?, ?, 'PENDING', ?)"
      ).bind(candidateId, agentId, cleanTitle, now).run();
