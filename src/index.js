...
app.get('/', async (c) => {
    try {
        const db = c.env.DB;
        // 1. Get Agent
        const agent = await db.prepare("SELECT * FROM agents LIMIT 1").first();
        if (!agent) {
            return c.html(`
...
app.post('/api/agent/init', async (c) => {
    try {
        ...
        const existingAgent = await c.env.DB.prepare("SELECT id FROM agents WHERE status = 'ACTIVE' LIMIT 1").first();
        if (existingAgent) {
        ...
        await c.env.DB.batch([
            c.env.DB.prepare("INSERT INTO agents (id, name, domain, status, created_at, updated_at) VALUES (?, ?, ?, 'ACTIVE', ?, ?)").bind(agentId, persona.name, persona.domain, now, now),
            c.env.DB.prepare("INSERT INTO runtime_state (agent_id, state, updated_at) VALUES (?, 'IDLE', ?)").bind(agentId, now)
        ]);
...
app.get('/api/agent/feed', async (c) => {
    ...
    try {
       ...
       FROM posts
       WHERE agent_id = ? AND status = 'PUBLISHED'
       ORDER BY created_at DESC`).bind(agentId).all();
